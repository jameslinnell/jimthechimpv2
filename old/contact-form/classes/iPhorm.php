        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->isValid($_POST)) {
                try {
                    ...
                    if ($this->getSuccessRedirect() !== null && !headers_sent()) {
                    	header('Location: ' . $this->getSuccessRedirect());
                    }
            ...
            if (isset($_POST['iphorm_submitted_by_js'])) {
                // The jQuery Form plugin in iframe mode recommends JSON to be wrapped in a textarea
                $response = '<textarea>' . $this->jsonEncode($result) . '</textarea>';
            } else {
                ob_start();
